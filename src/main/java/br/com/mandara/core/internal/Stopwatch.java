...
public final class Stopwatch {
    ...
    public static synchronized void beginStopwatch(String key) {
        if (!enabled) 
            return;
        watches.put(key, new Stopwatch(key).begin());
    }
    
    public static synchronized void endStopwatch(String key) {
        if (!enabled) 
            return;
        watches.get(key).end();
    }
